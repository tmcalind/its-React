...
const AddressMap = ({ basemap, height, width, center, scale }) => {
  ...
  useEffect(() => {
    if (mapDiv.current) {
      ...
      const addressLayerUrl =
        'https://citymap/arcgisa/rest/services/References/AddressMaster/MapServer/0';
      ...
      addressLayer.when(() => {
        view
          ...
          .then((layerView) => (addressSelectionLayerView = layerView));
      });
      ...
      // add parcels
      const parcelsLayerUrl =
        'https://citymap/arcgisa/rest/services/References/ParcelMapping/MapServer/0';
            ...
            addresses = featureSet.features.map((feature) => {
              return {
                ...
                MunicipalNumberQualifier:
                  feature.attributes.MunicipalNumberQualifier,
      ...
      sketchViewModel.on('create', async (event) => {
        if (event.state === 'complete') {
          const geometries = selectionSketchLayer.graphics.map(
